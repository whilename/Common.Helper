...
namespace ThoughtWorks.QRCode.Codec.Reader.Pattern
{
    ...
    {
        internal static readonly int[] VersionInfoBit = new int[34] { 31892, 34236, 39577, 42195, 48118, 51042, 55367, 58893, 63784, 68472, 70749, 76311, 79154, 84390, 87683, 92361, 96236, 102084, 102881, 110507, 110734, 117786, 119615, 126325, 127568, 133589, 136944, 141498, 145311, 150283, 152622, 158308, 161089, 167017 };
        ...
        public virtual int SqrtNumModules { get { return 17 + 4 * this.version; } }
        ...
        public static FinderPattern findFinderPattern(bool[][] image)
        {
            ...
            Point[] center;
            try { center = FinderPattern.getCenter(lineCross); }
            catch (FinderPatternNotFoundException ex) { throw ex; }
            ...
            int[] moduleSize = new int[3] { (width[0] << QRCodeImageReader.DECIMAL_POINT) / 7, (width[1] << QRCodeImageReader.DECIMAL_POINT) / 7, (width[2] << QRCodeImageReader.DECIMAL_POINT) / 7 };
            int version = FinderPattern.calcRoughVersion(pointArray, width);
            if (version > 6)
            {
                try { version = FinderPattern.calcExactVersion(pointArray, angle, moduleSize, image); }
                catch (VersionInformationException ex) { }
            }
            return new FinderPattern(pointArray, version, angle, width, moduleSize);
        }

        public virtual Point[] getCenter() { return this.center; }
        ...
        public virtual int getWidth(int position) { return this.width[position]; }

        public virtual int[] getAngle() { return this.sincos; }

        public virtual int getModuleSize() { return this.moduleSize[0]; }

        public virtual int getModuleSize(int place) { return this.moduleSize[place]; }
        ...
        internal static int[] getAngle(Point[] centers)
        {
            ...
            return new int[2] { (p2.Y - p1.Y << QRCodeImageReader.DECIMAL_POINT) / length, (p2.X - p1.X << QRCodeImageReader.DECIMAL_POINT) / length };
        }
        ...
        internal static int calcExactVersion(Point[] centers, int[] angle, int[] moduleSize, bool[][] image)
        {
            ...
            int num;
            try { num = FinderPattern.checkVersionInfo(target); }
            catch (InvalidVersionInfoException ex1)
            {
                ...
                FinderPattern.canvas.drawPoints(points, Color_Fields.RED);
                try { num = FinderPattern.checkVersionInfo(target); }
                catch (VersionInformationException ex2) { throw ex2; }
            }
            return num;
        }

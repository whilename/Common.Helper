...
namespace ThoughtWorks.QRCode.Codec.Data
{
    public class QRCodeSymbol
    {
        ...
        public virtual int NumErrorCollectionCode { get { return this.numErrorCollectionCode[this.version - 1][this.errorCollectionLevel]; } }

        public virtual int NumRSBlocks { get { return this.numRSBlocks[this.version - 1][this.errorCollectionLevel]; } }

        public virtual int Version { get { return this.version; } }

        public virtual string VersionReference
        {
            get
            {
                return Convert.ToString(this.version) + (object)"-" + (string)(object)new char[4] { 'L', 'M', 'Q', 'H' }[this.errorCollectionLevel];
            }
        }

        public virtual Point[][] AlignmentPattern { get { return this.alignmentPattern; } }

        public virtual int DataCapacity { get { return this.dataCapacity; } }

        public virtual int ErrorCollectionLevel { get { return this.errorCollectionLevel; } }

        public virtual int MaskPatternReferer { get { return this.maskPattern; } }
        ...
        public virtual int[] Blocks
        {
            get
            {
                ...
                do
                {
                    ...
                    if (this.getElement(num1, num2)) { num3 += 1 << num4; }
                    --num4;
                    if (num4 == -1) { arrayList2.Add((object)num3); num4 = 7; num3 = 0; }
                    do
                    {
                        if (flag3 == flag1)
                        {
                            if ((num1 + num5) % 2 == 0) { --num1; }
                            else if (num2 > 0) { ++num1; --num2; }
                            else
                            {
                                --num1;
                                if (num1 == 6) { --num1; num5 = 1; }
                                flag3 = flag2;
                            }
                        }
                        else if ((num1 + num5) % 2 == 0) { --num1; }
                        else if (num2 < height - 1) { ++num1; ++num2; }
                        else
                        {
                            --num1;
                            if (num1 == 6) { --num1; num5 = 1; }
        ...
        public virtual bool getElement(int x, int y) { return this.moduleMatrix[x][y]; }
        ...
        internal virtual void unmask()
        {
            ...
            for (int y = 0; y < width; ++y)
            {
                ...
                {
                    if (flagArray[x][y]) { this.reverseElement(x, y); }
                }
            }
        }
                    ...
                    if (!this.isInFunctionPattern(targetX, targetY))
                    {
                        switch (maskPatternReferer)
                        {
                            case 0:
                                ...
                                {
                                    flagArray[targetX][targetY] = true; break;
                                }
                                break;
                            case 1:
                                if (targetY % 2 == 0)
        ...
        public virtual void reverseElement(int x, int y) { this.moduleMatrix[x][y] = !this.moduleMatrix[x][y]; }

        public virtual bool isInFunctionPattern(int targetX, int targetY)
        {
            if (targetX < 9 && targetY < 9 || targetX > this.Width - 9 && targetY < 9 || targetX < 9 && targetY > this.Height - 9 
                || this.version >= 7 && (targetX > this.Width - 12 && targetY < 6 || targetX < 6 && targetY > this.Height - 12) || (targetX == 6 || targetY == 6))
                return true;
            ...
            for (int index1 = 0; index1 < length; ++index1)
            {
                ...
                {
                    if ((index2 != 0 || index1 != 0) && (index2 != length - 1 || index1 != 0) && (index2 != 0 || index1 != length - 1) 
                        && (Math.Abs(alignmentPattern[index2][index1].X - targetX) < 3 && Math.Abs(alignmentPattern[index2][index1].Y - targetY) < 3))
                        return true;
                }
            }
            return false;
